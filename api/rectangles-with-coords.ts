...
import { RectangleEntrySkeleton } from '../lib/contentful/rectangle-entry';

const getRectanglesWithCoords = async (req: VercelRequest, res: VercelResponse) => {
  const client = createCDAClient();

  const entryCollection = await client.withoutUnresolvableLinks.getEntries<RectangleEntrySkeleton>({
